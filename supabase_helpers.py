# supabase_helpers.py (fixed upload)
...
def upload_export_bytes(content: bytes, path: str, expires_seconds: int = 86400) -> str:
    """Upload bytes to Storage and return a signed URL (Supabase Storage)."""
    bucket = os.environ.get("SUPABASE_BUCKET", "exports")
    sb = sb_client()
    # Correct call signature: upload(path, file, file_options)
    sb.storage.from_(bucket).upload(path, content, {"contentType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "upsert": True})
