# timesheet_app.py — Excel workflow + SharePoint upload + "Download All" ZIP
# Keeps your original export formats and adds a single button to download all created exports at once.
#
# Prereqs (unchanged):
# - Azure App Registration with Graph app perms: Files.ReadWrite.All, Sites.ReadWrite.All (admin consent)
# - Secrets via env vars or Streamlit secrets:
#     TENANT_ID, CLIENT_ID, CLIENT_SECRET
#     SP_SITE="https://<tenant>.sharepoint.com/sites/<YourSite>"
#     SP_DRIVE="Documents"
#     SP_EXPORT_FOLDER="Exports"
#
# Files in the app folder:
#   TimeSheet Apps.xlsx, Daily Time.xlsx, sharepoint_upload.py
#   (optional) timesheet_default_path.txt -> full path to TimeSheet Apps.xlsx

import os, json, datetime as dt, io, zipfile
...
st.set_page_config(page_title="Timesheet (Excel + SharePoint)", page_icon=":clipboard:", layout="centered")
...
with st.sidebar:
    ...
    st.caption("SharePoint export: " + ("ENABLED" if USE_SHAREPOINT else "OFF"))
...
def export_all_jobs_for_date(xlsx: str, date_str: str, outdir: str, user: str, paycode_map: Dict[str,str]):
    ...
    # SharePoint path root (if enabled)
    sp_root = os.environ.get("SP_EXPORT_FOLDER","Exports").strip("/") if USE_SHAREPOINT else None

    # collect bytes for "Download All"
    downloads = []  # list of (filename, bytes)
    ...
    for job in jobs:
        ...
        # Write to disk (local copy) and also collect bytes
        write_formatted(out_df, out_path)
        out_paths.append(out_path)
        try:
            with open(out_path, "rb") as f:
                downloads.append((file_name, f.read()))
        except Exception:
            pass
        ...
        if USE_SHAREPOINT:
            try:
                sp_path = f"{sp_root}/{month}/{file_name}"
                share_link = upload_export_to_sharepoint(sp_path, downloads[-1][1], link_scope="organization")
                st.success(f"Uploaded to SharePoint: {sp_path}")
                st.link_button("Open in SharePoint", share_link, use_container_width=True)
            except Exception as e:
                st.error(f"SharePoint upload failed for {file_name}: {e}")

        ws_log.append([f"{date_str}-{job}-{datetime.utcnow().strftime('%H%M%S')}", date_str, job, len(out_df), out_path.name, str(out_path.parent), share_link, user, datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"), "Created", ""])
    wb.save(xlsx)
    return out_paths, downloads
...
def export_daily_time_report(xlsx: str, template_path: str, date_str: str, outdir: str, user: str=""):
    ...
    if "Date" not in td.columns: return None, None
    ...
    try:
        ...
        out_name = f"{date_obj.strftime('%m-%d-%Y')} – Daily Time.xlsx"  # en dash
        out_path = outdir_p/out_name
        wb.save(out_path)

        # Read bytes (for bulk download)
        try:
            with open(out_path, "rb") as f:
                file_bytes = f.read()
        except Exception:
        ...
        share_link = ""
        if USE_SHAREPOINT and file_bytes is not None:
            try:
                sp_root = os.environ.get("SP_EXPORT_FOLDER","Exports").strip("/")
                sp_path = f"{sp_root}/{date_obj.strftime('%B')}/{out_name}"
                share_link = upload_export_to_sharepoint(sp_path, file_bytes, link_scope="organization")
                st.success(f"Uploaded to SharePoint: {sp_path}")
                st.link_button("Open daily report (SP)", share_link, use_container_width=True)
            except Exception as e:
                st.error(f"SharePoint upload failed for daily report: {e}")
        ...
        wslog.append([f"{date_str}-DAILY-{datetime.utcnow().strftime('%H%M%S')}", date_str, "ALL", len(day), out_path.name, str(outdir_p), share_link, user, datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"), "Created", ""])
        logwb.save(xlsx)
        return str(out_path), (out_name, file_bytes)
    except Exception:
        return None, None
...
if st.session_state.get("is_admin", False):
    ...
    outdir_input = st.text_input("Export Directory (local copy)", value=default_outdir, key="export_outdir")
    ...
    with st.form("export_form"):
        ...
        if do_export:
            ...
            try:
                paths, dl_parts = export_all_jobs_for_date(
                    xlsx=xlsx_path,
                ...
                daily_path, daily_part = export_daily_time_report(
                    xlsx=xlsx_path,
                ...
                # Build a single ZIP for "Download All"
                all_files = list(dl_parts)
                if daily_part and daily_part[1] is not None:
                    all_files.append(daily_part)
                if all_files:
                    zip_buf = io.BytesIO()
                    ...
                    zip_buf.seek(0)
                    zip_name = f"Exports_{export_date.strftime('%m-%d-%Y')}.zip"
                    st.download_button("Download All Exports (.zip)", data=zip_buf.getvalue(),
                                       file_name=zip_name, mime="application/zip", use_container_width=True)
...
st.caption("Exports are saved locally and (if configured) uploaded to SharePoint. A single 'Download All' ZIP is provided for convenience.")
