# timesheet_app.py — v25.0
# - Fix: "Delete selected rows" now works reliably.
#   We replaced the Excel row-matching with a robust normalizer that tolerates
#   dates/strings/floats/blank differences and maps each displayed row to the
#   correct Excel row number.
# - Keeps: landing page (original style), "Delete ALL" option, active costcode filter,
#   session_state hardening, and admin export.
...
def _norm_val(v):
    ...
    try:
        # treat numeric-like consistently
        f = float(v)
        ...
        s = str(v).strip()
        # common date-like strings to yyyy-mm-dd
...
def map_rows_to_excel_indices(xlsx_file: str, filt_df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        # Build normalized list of excel rows
        excel_rec = []
        ...
        for _, r in filt_df.iterrows():
            ...
            for ridx, rec in excel_rec:
                ...
                for c, val in target.items():
                    if val == "":  # ignore blanks to be forgiving
                        continue
                    if rec.get(c, "") != val:
                        ok = False; break
                if ok:
