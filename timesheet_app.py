...
def export_daily_time_report(xlsx: str, template_path: str, date_str: str, outdir: str, user: str = "") -> str:
    ...
    try:
        ...
        # Save file
        out_dir = str(Path(outdir)).replace("/", "\\")  # Convert to string and replace slashes
        st.write(f"Export directory resolved to: {out_dir}")  # Debug: Show resolved export directory
        ...
        try:
            wb.save(out_path)
            st.write(f"File saved to: {out_path}")  # Confirm save success
            # Verify file attributes
            if os.path.exists(out_path):
                file_size = os.path.getsize(out_path)
                file_time = os.path.getmtime(out_path)
                st.write(f"File verified at: {out_path} (Size: {file_size} bytes, Modified: {datetime.fromtimestamp(file_time).strftime('%Y-%m-%d %H:%M:%S')})")
                # Create a backup copy in a local temp directory
                temp_dir = os.path.join(os.environ["TEMP"], "DailyTimeBackup")
                os.makedirs(temp_dir, exist_ok=True)
                backup_path = os.path.join(temp_dir, out_name)
                ...
                try:
                    ...
                    st.write(f"Successfully opened file at: {out_path}")
                except subprocess.CalledProcessError as e:
                    st.warning(f"Failed to open file at {out_path}: {e}")
            else:
                st.error(f"File not found at: {out_path} after save attempt")  # Warn if file is missing
        except PermissionError as pe:
            st.error(f"Permission denied saving to {out_path}: {pe}")
        except Exception as e:
            st.error(f"Save operation failed: {e}")
        # Log to Exports Log
