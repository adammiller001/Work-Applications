...
def export_descriptions_for_date(xlsx: str, date_str: str, outdir: str, user: str):
    ...
    for job in sorted(subset["Job Number"].astype(str).str.strip().unique().tolist()):
        ...
        # File names use mm-dd-yyyy format per user request
        file_name = f"{dt_obj.strftime('%m-%d-%Y')} - {job} - Description.xlsx"
        out_path = Path(outdir) / month_folder / file_name
        out_path.parent.mkdir(parents=True, exist_ok=True)
        import xlsxwriter
...
with st.form("export_form"):
    ...
    # Always export to the directory containing the workbook.  A month-named
    ...
    if do_export:
        outdir = str(Path(xlsx_path).parent)
        try:
            ...
            desc_paths = export_descriptions_for_date(
                xlsx=xlsx_path,
                date_str=export_date.strftime("%Y-%m-%d"),
                outdir=outdir,
                user=user,
            ...
            if total == 0:
                ...
                st.success(
                    f"Created {total} file(s): {len(paths)} TimeEntries and {len(desc_paths)} Description file(s)."
                )
        except Exception as e:
