# timesheet_app.py — Excel workflow with external template (TimeEntries.xlsx) + Daily report + optional SharePoint
# v25.3
#
# Changes in this build
# - Per‑job “Daily Time Import.xlsx” exports now use the external template **TimeEntries.xlsx**
#   that sits beside this file. The sheet named **TimeEntries** in that workbook dictates
#   the header colors, fonts, column widths, row heights, etc.
# - Daily Time report still uses **Daily Time.xlsx** (beside the app) and is populated as before.
# - Still offers per‑file Download buttons and optional SharePoint upload.
#
# Place in the same folder:
#   - TimeEntries.xlsx       (export formatting template; must contain a sheet named "TimeEntries")
#   - Daily Time.xlsx        (daily report template)
#   - TimeSheet Apps.xlsx    (data workbook: Employee List, Job Numbers, Cost Codes, Time Data, etc)
#   - sharepoint_upload.py   (optional, for SharePoint uploads)
#
# Optional:
#   - timesheet_default_path.txt : full path to "TimeSheet Apps.xlsx" if it’s elsewhere.
#
# Requirements: streamlit, pandas, openpyxl, xlsxwriter
# SharePoint (optional): office365-rest-python-client and valid SP_* secrets
...
TEMPLATE_EXPORT_BOOK = APP_DIR / "TimeEntries.xlsx"   # <— NEW: external template for per-job exports
DAILY_TEMPLATE_BOOK  = APP_DIR / "Daily Time.xlsx"

# ---------- Initialize session keys (avoid KeyError) ----------
for k, v in {
    "whoami_email": "",
    "entered_app": False,
    "is_admin": True,   # keep exports visible; change to Users gating later if you like
...
# Lookups + template sheet (TimeEntries) come from the data workbook
...
def export_per_job_with_template(time_data_df: pd.DataFrame, job: str, export_date: date) -> io.BytesIO:
    # Build data
    subset = time_data_df[time_data_df["Job Number"].astype(str).str.strip() == str(job)].copy()
    out_df = build_timeentries_df(subset)

    # Load external template (beside app)
...
def export_daily_report_populated(xlsx: str, template_path: Path, date_str: str, user: str="") -> io.BytesIO:
    # Read day subset
    ...
    try:
        ...
        ws["B2"] = "2224138065"  # Job Number (as per your spec)
...
if do_export:
    ...
    if td.empty or "Date" not in td.columns:
        ...
        if day_df.empty:
            ...
            # (A) Per‑job TimeEntries exports (using external template)
            jobs_for_day = sorted(day_df["Job Number"].astype(str).str.strip().unique().tolist())
            n_files = 0
            for job in jobs_for_day:
                try:
                    buf = export_per_job_with_template(day_df, job, export_date)
            ...
            if not DAILY_TEMPLATE_BOOK.exists():
                ...
                try:
                    out2 = export_daily_report_populated(xlsx_path, DAILY_TEMPLATE_BOOK, export_date.strftime("%Y-%m-%d"), st.session_state.get("whoami_email",""))
                    if out2 is not None:
...
st.caption("Per‑job exports are styled by TimeEntries.xlsx → 'TimeEntries' sheet. Daily report uses Daily Time.xlsx. Both download locally; SharePoint upload is optional.")
