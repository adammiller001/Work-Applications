...
st.set_page_config(page_title="Timesheet Entry", page_icon=":clock3:", layout="centered")
...
if not st.session_state.get("entered_app", False):
    # Vertical spacing to place content roughly mid/upper-mid
    st.write(""); st.write(""); st.write("")
    top_spacer, mid_col, right_spacer = st.columns([1,2,1])
    with mid_col:
        # Logo next to the script
        _img = None
        for cand in ["PTW.jpg","PTW-Square.jpg","PTW.png"]:
            try:
                _pth = Path(__file__).with_name(cand)
            except NameError:
                _pth = Path(cand)
            if _pth.exists():
                _img = str(_pth); break
        if _img:
            st.image(_img, use_container_width=False)

        st.write("")
        default_email = st.session_state.get("whoami_email", os.getenv("USER_EMAIL",""))
        email_val = st.text_input("Your work email", value=default_email, key="home_user_email")
        if st.button("Enter", key="home_enter_btn"):
            st.session_state["whoami_email"] = email_val.strip()
            try:
                default_xlsx_here = str(Path(__file__).with_name("TimeSheet Apps.xlsx"))
            except NameError:
                default_xlsx_here = "TimeSheet Apps.xlsx"
            if not st.session_state.get("xlsx_path"):
                st.session_state["xlsx_path"] = default_xlsx_here
